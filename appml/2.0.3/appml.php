...
if ($txt != "") {
  ...
  $loginmessage = "";
  if ($userAccess != "") {
    $nn = app_setSession($userName, $userAccess);
    $loginmessage = "OK";
  }
  echo "{";
...
if ($dataSource == "") {
  app_error("APPML_ERR_DATASOURCE_REQ");
}
if ($action == "GET") {
...
if ($action != "UPDATE" and $action != "ADD" and $action != "DELETE") {
  app_error("APPML_ERR_ILLEGAL_ACTION: " . $action);
}
...
if ($action == "DELETE") {
  if (strtoupper(app_getProperty($txtModel, "delete")) == "FALSE") {
    app_error("APPML_ERR_ILLEGAL_ACTION: " . "DELETE");
  }
...
if ($action == "ADD") {
  if (strtoupper(app_getProperty($txtModel, "addnew")) == "FALSE") {
    app_error("APPML_ERR_ILLEGAL_ACTION: " . "INSERT");
  }
}
$fieldCounter = 0;
...
function appml_GetQueryWhere($txtParam, $filter, $requestFilter, &$paramCount, &$params)
{
  ...
  for ($j = 0; $j < $l; $j++) {
    ...
    if ($queryOK == 0) {
      app_error("APPML_ERR_ILLEGAL_QUERY: " . $qname);
    }
...
function app_isDate($x)
{
  $x = true;
}

function app_cDate($x)
{
  return strtotime($x);
}
...
function app_getData(
  ...
  if ($keyValue != "") {
    if ($keyCounter == 0) {
      app_error("APPML_ERR_KEYFIELD_REQ");
    }
    if ($mainTable == "") {
      app_error("APPML_ERR_MAINTABLE_REQ");
    }
    if ($keyValue == "NULL") {
      $keyValue = "-1";
    }
